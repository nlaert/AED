...
public class IterablesTest {
	...
	protected static <T> void assertIterableEquals(Iterable<T> expected,
			Iterable<T> result) {
		Iterator<T> itE = expected.iterator(), itR = result.iterator();
		while (itE.hasNext()) {
	...
	protected static <T> void testNextWithoutHasNext(Iterable<T> expected,
			Iterable<T> result) {
		Iterator<T> itE = expected.iterator(), itR = result.iterator();
		while (itE.hasNext()) {
			assertEquals(itE.next(), itR.next());
	...
	protected static <T> void testMultipleIterations(Iterable<T> expected,
			Iterable<T> result) {
		Iterator<T> itR, itE;
		for (int i = 0; i < 2; ++i) {
	...
	protected static <T> void testDoubleHasNext(Iterable<T> expected,
			Iterable<T> result) {
		Iterator<T> itE = expected.iterator(), itR = result.iterator();
		while (itR.hasNext()) {
			assertTrue(itR.hasNext());
