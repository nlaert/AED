...
import static serie2.ListUtils.quicksort;
import static serie2.TestListUtil.isSorted;
import static serie2.TestListUtil.getRandomList;
...
public class QuickSortTest {
	...
	public void quicksort_with_increasingElements() {
		Node<Integer> list = TestListUtil.getListWithoutSentinel(0, 20, 1);
		for (int i = 0; i < 20; i++) {
			Node<Integer> last = TestListUtil.getNNode(i, list);
	...
	public void quicksort_with_SomeElements1_list() {
		...
		array.add(-34);
		array.add(-36);
		Node<Integer> list = TestListUtil.getListWithoutSentinel(array);
		Node<Integer> last = TestListUtil.getNNode(6, list);
	...
	public void quicksort_with_som2Elements2_list() {
		...
		array.add(-19);
		Node<Integer> list = TestListUtil.getListWithoutSentinel(array);
		Node<Integer> last = TestListUtil.getNNode(10, list);
		quicksort(list, last, CMP_NATURAL_ORDER);
		assertTrue(isSorted(list, last, CMP_NATURAL_ORDER));
	}
